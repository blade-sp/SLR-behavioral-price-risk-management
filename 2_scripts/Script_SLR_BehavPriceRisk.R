...
# load libraries
requiredPackages <- c("tidyverse", "readxl", "networkD3", "ggbreak",
                       "stringr", "patchwork", "RColorBrewer",
                       "pagedown", "grid", "writexl")
...
df <- read_excel("1_Data/Data_SLR_BehavPriceRisk.xlsx", sheet = "data", skip = 2)
...
study_design <- c("theoretical_model", "optimization", "econometric_method",
                  "experiment")

model <- c("max_EU", "M_V", "max_profits", "stochastic_dominance",
           "prospect_theory", "bayesian", "agency_theory", "VaR")

tools <- c("forward", "futures", "options", "price_insurance", "swap",
           "multiple_tools")

behav_pref <- c("risk_preferences", "uncertainty_aversion",
                "variation_aversion", "reference_dependence",
                "loss_aversion", "probability_weighting",
                "time_preference", "price_expectation_theor")

psyc_factors <- c("risk_attitudes", "risk_perceptions",
                  "price_yield_expectation", "alternative_perceptions",
                  "network_influence", "subjective_norms",
                  "management_attitudes", "innovativeness",
                  "perceived_usefulness", "tools_knowledge")

oper <- c("predicted", "self_reported_intention", "self_reported_adoption",
          "actual_adoption")
...
other_strategies <- c("cash_marketing", "storage", "diversification",
                      "government_programs", "non_price_insurance",
                      "other_risk_sharing_strategies", "leverage", "nothing")


# function that transforms data from horizontal to vertical
pivot_columns <- function(data, cols_to_pivot, names_col, values_col = "value") {
  data %>%
    pivot_longer(
...
create_behavior_table <- function(data, behavior_col, behav_pref, farm_type, tools) {
  data %>%
    ...
    select("authors", "year", "study_loc", "farm_type", "farm_type_notes",
           "tool_considered", "model", "sign") %>%
    arrange(tool_considered)
}

# function that creates a summary table for each psychological factor
create_psych_table <- function(data, psych_col, psyc_factors, farm_type, tools) {
  data %>% 
    ...
    select("authors", "year", "study_loc", "farm_type", "farm_type_notes",
           "tool_considered", "operationalization", "sign", "significance") %>%
    arrange(tool_considered)
}
...
df <- df %>%
  ...
  mutate(across(all_of(c(farm_type, study_type, study_design, model, tools, 
                         behav_pref, psyc_factors, oper, risk_type, direction, 
                         significance, other_strategies)), ~ ifelse(is.na(.), 0, .)))
...
df_year <- df %>% 
  ...
  complete(year = full_seq(df$year, 1), contains_psych_factor, fill = list(count = 0))  

# plot the distribution
year_distribution_plot <- ggplot(df_year, aes(x = year, y = count, fill = contains_psych_factor)) +  
  geom_col(position = "stack") + 
  labs(x = "", y = "Number of papers") +
  scale_fill_manual(values = c("#5788C4", "#D79E42"),
                    name = "Type of study",
                    labels = c("Behavioral preference",
...
nodes <- data.frame(
  name = c(as.character(pair_counts$model),
...
all_links <- bind_rows(
  ...
  pair_counts2 %>% select(Source = IDpreference, Target = IDmodel, Value = count)
)
...
behav_pair_counts <- lapply(behav_pairs, function(vars) {
  ...
  bind_rows(tibble(factor = "total_behav_preference", tool = "any", 
...
psych_pair_counts <- lapply(psych_pairs, function(vars) {
  ...
  bind_rows(tibble(factor = "total_psyc_factors", tool = "any", 
...
behav_plot <- ggplot(behav_pair_counts, aes(x = fct_reorder(factor, cumsum_obs), y = obs, fill = tool)) +
  ...
  scale_fill_brewer(palette = "Paired",
                    name = "Financial tool") +
  geom_text(data = behav_pair_counts_total,
...
psych_plot <- ggplot(psych_pair_counts, aes(x = fct_reorder(factor, cumsum_obs), y = obs, fill = tool)) +
  geom_bar(position = "stack", stat = "identity", width = 0.5) +
  ...
  scale_fill_brewer(palette = "Paired") +
  geom_text(data = psych_pair_counts_total,
            aes(x = factor, y = total, label = total),
            inherit.aes = FALSE,  # Prevent geom_text from using aesthetics in ggplot()
            position = position_nudge(y = 2.5),  # Slightly nudge the label upward
            size = 3) +
...
overview_plot <- behav_plot + psych_plot + 
...
risk_preferences_table <- create_behavior_table(df, "risk_preferences", behav_pref, farm_type, tools)

loss_aversion_table <- create_behavior_table(df, "loss_aversion", behav_pref, farm_type, tools)

probability_weighting_table <- create_behavior_table(df, "probability_weighting", behav_pref, farm_type, tools)

uncertainty_aversion_table <- create_behavior_table(df, "uncertainty_aversion", behav_pref, farm_type, tools)

variation_aversion_table <- create_behavior_table(df, "variation_aversion", behav_pref, farm_type, tools)

time_preference_behav <- create_behavior_table(df, "time_preference", behav_pref, farm_type, tools)

reference_dependence_behav <- create_behavior_table(df, "reference_dependence", behav_pref, farm_type, tools)

price_exp_behav <- create_behavior_table(df, "price_expectation_theor", behav_pref, farm_type, tools)
...
behavioral_preference_table <- bind_rows(
  ...
  mutate(farm_type_notes = ifelse(is.na(farm_type_notes), "not specified", farm_type_notes),
  ...
  select(c("Preference", "tool_considered", "authors", "study_loc", "farm_type",
           "farm_type_notes", "model", "sign")) %>%
  arrange(Preference, tool_considered, model) %>% 
  rename(Tool = tool_considered, Study = authors, Country = study_loc, 
         "Farm type" = farm_type, "Product" = farm_type_notes, Model = model, Direction = sign)
...
behav_data <- behavioral_preference_table %>% 
  mutate(Direction = case_when(Direction == "+" ~ 1,
                               Direction == "-" ~ -1,
                               Direction == "0" ~ 0),
         Preference = case_when(Preference == "risk_preferences" ~ "Risk preferences",
                                Preference == "uncertainty_aversion" ~ "Uncertainty aversion",
                                Preference == "variation_aversion" ~ "Variation aversion",
                                Preference == "reference_dependence" ~ "Reference dependence",
                                Preference == "loss_aversion" ~ "Loss aversion",
...
behav_data_pct <- behav_data %>%
  ...
  mutate(Direction_cat = case_when(
    Direction > 0 ~ "Positive",
    Direction < 0 ~ "Negative",
    TRUE ~ "Insignificant"
  )) %>%
  mutate(Direction_cat = factor(Direction_cat, levels = c("Positive",
                                                          "Insignificant", 
                                                          "Negative" 
                                                          )))

# Create the plot
behav_pref_percentage <- ggplot(data = behav_data_pct, 
                                aes(x = Preference, y = Count, fill = Direction_cat)) +
  geom_col(width = 0.6) +
  coord_flip() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)), 
                     breaks = seq(0, 60, by = 5)) +
  scale_x_discrete(limits = rev(c("Risk preferences", "Time preference", "Reference dependence", 
                              "Loss aversion", "Price expectations", "Probability weighting", 
                              "Variation aversion", "Uncertainty aversion"))
                            ) +
  scale_fill_manual(values = c("Positive" = "#ce9d41ff",
                               "Insignificant" = "gray70", 
                               "Negative" = "#482f57ff" 
                               ),
                   name = NULL) +
  geom_text(data = behav_data_pct %>% distinct(Preference, Total),
            aes(x = Preference, y = Total, label = paste0("n = ", Total)),
            inherit.aes = FALSE, hjust = -0.4, size = 3.5, fontface = "bold") +
...
risk_attitude_table <- create_psych_table(df, "risk_attitudes", psyc_factors, farm_type, tools)

management_attitudes_table <- create_psych_table(df, "management_attitudes", psyc_factors, farm_type, tools)

innovativeness_table <- create_psych_table(df, "innovativeness", psyc_factors, farm_type, tools)
...
dispositional_factor_table <- bind_rows(
  ...
  mutate(Direction = case_when(
         ...
         Operationalization = case_when(
           operationalization == "self_reported_intention" ~ "Self reported intention",
  ...
  select(c("Factor", "tool_considered", "Study", "Country", "farm_type",
          "Product",  "Operationalization", "Direction")) %>% 
...
network_influence_table <- create_psych_table(df, 'network_influence', psyc_factors, farm_type, tools)

subjective_norms_table <- create_psych_table(df, "subjective_norms", psyc_factors, farm_type, tools)
...
risk_perception_table <- create_psych_table(df, "risk_perceptions", psyc_factors, farm_type, tools)

price_yield_expectations_psych <- create_psych_table(df, "price_yield_expectation", psyc_factors, farm_type, tools)

alternative_perception_table <- create_psych_table(df, "alternative_perceptions", psyc_factors, farm_type, tools)

perceived_usefulness_table <- create_psych_table(df, "perceived_usefulness", psyc_factors, farm_type, tools)

tools_knowledge_table <- create_psych_table(df, "tools_knowledge", psyc_factors, farm_type, tools)


cognitive_factor_table <- bind_rows(
  risk_perception_table %>% mutate(Factor = "Risk perceptions"),
  price_yield_expectations_psych %>% mutate(Factor = "Price and yield expectations"),
  alternative_perception_table %>% mutate(Factor = "Alternative tools perceptions"),
...
psych_data <- bind_rows(dispositional_factor_table, social_factor_table, cognitive_factor_table) %>% 
  mutate(Direction = case_when(
    Direction == "+" ~ 1,
    Direction == "-" ~ -1,
    Direction == "0" ~ 0))
...
psych_percentage <- psych_data %>% 
  group_by(Factor) %>% 
  summarise(tot_stat_sign = sum(Direction != 0),
            count = n(),
            share_stat_sign = tot_stat_sign/sum(count)) %>% 
  mutate(share_tot = count/sum(count),
         share_stat_sign_test = share_stat_sign*share_tot)

psych_factor_share <- ggplot(data = psych_percentage) +
  geom_col(aes(x = Factor, y = count, fill = "Total Observations"), alpha = 0.7, width = 0.5) +  # First bar color
  geom_col(aes(x = Factor, y = tot_stat_sign, fill = "Statistically Significant"), alpha = 0.7, width = 0.5) +  # Second bar color
  ...
  scale_fill_manual(values = c("Total Observations" = "lightgrey", "Statistically Significant" = "#5788C4")) +  # Custom colors
  ...
  annotate("text", x = 4.5, y = 40, label = "Cognitive", size = 3, color = "black") +
  geom_vline(xintercept = 7.5, color = "black", size = 0.3, linetype = "solid") +
  annotate("text", x = 6.5, y = 40, label = "Social", size = 3, color = "black") +
  geom_vline(xintercept = 10.5, color = "black", size = 0.3, linetype = "solid") +
  annotate("text", x = 9, y = 40, label = "Dispositional", size = 3, color = "black") 
...
psych_factor_absolute <- ggplot(data = psych_data) +
  ...
  scale_fill_manual(
    values = c("Self reported intention.1" = "#D79E42", "Self reported intention.-1" = "#E7B769",   
               "Self reported adoption.1" = "#5788C4", "Self reported adoption.-1" = "#79AAD9",   
               "Actual adoption.1" = "#5A9C87", "Actual adoption.-1" = "#72B7A0"), 
    name = "Operationalization",
    breaks = c("Self reported intention.1", 
